...
RUN echo "deb http://kali.download/kali kali-last-snapshot main contrib non-free" > /etc/apt/sources.list && \
   ...
   DEBIAN_FRONTEND=noninteractive apt-get install -y nano curl wget git ncat dbus-x11 python2 python3 python3-pip p7zip-full p7zip-rar p7zip unzip net-tools build-essential iputils-ping pciutils iproute2 bash-completion kali-linux-default pcmanfm leafpad terminator tmux && \
